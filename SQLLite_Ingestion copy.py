...
def process_files_for_tier(file_list_or_single_file, tier_key, table_dict):
    ...
    for file_name in files_to_process:
        if file_name:
            ...
            # The process_files_for_tier function is used to create the JSON config structure 
            # where the key is the table name and the value is the database file it should reside in.
            db_file_name = f"{tier_key}.db" 
...
def run_part1_fetch_files():
    ...
    file_config = {}
    if csv_files["BRONZE"]:
        # Only takes the first file found in the BRONZE folder
    ...
    try:
        ...
        config_data = {"databases": {}, "logging": {"log_file": "default_db_operations.log"}} # Initialize basic structure

    existing_files = config_data.get("files", {})
    existing_files.update(file_config)
    config_data["files"] = existing_files
    
    # Ensure database files are explicitly set in the config for the metadata function to find them
    ...
    print(f"Writing updated filenames and database files to {JSON_FILE_PATH}...")
...
def load_data_from_csv_to_db():
    ...
    # Use standard database file names as they are created in the current directory
    db_map = {
        "bronze": "bronze.db",
        "silver": "silver.db",
        "gold": "gold.db",
...
def main():
    """Orchestrates the entire data pipeline: Fetching files, creating tables, loading data, and extracting metadata."""
    run_part1_fetch_files()
    run_part2_create_tables() 
    load_data_from_csv_to_db()
    
    # 🟢 NEW: Run the metadata extraction and CSV compilation after loading the data
